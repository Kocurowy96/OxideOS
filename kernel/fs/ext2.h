...
// Faza 1a/1b/1c/1d/1e (patrz CoworkWithClaude/PLAN_ext2_filesystem.md): Init() parsuje
...
// bezposrednich (block[0..11]) - wieksze pliki (blok posredni) to Faza 1f. WriteFile
// na razie sama deklaracja (Faza 2). Odczyt i-wezla (Ext2Inode/ReadInode) i
// rozwiazywanie sciezek (ResolvePath, parser wpisow katalogowych) zyja na razie tylko
// w ext2.cpp. Sterownik NIE jest jeszcze podlaczony do VFS:: - disk.img zostaje
// FAT32-owy, testujemy na osobnym obrazie mke2fs podlaczonym jako drugi dysk QEMU
// (patrz ext2.cpp).
class Ext2 {
public:
    static void Init();
