...
- [ ] Dodać brakujący `#include <stdbool.h>` w `apps/calculator/main.c` — zgłoszone dwukrotnie przez agenta (2026-09-14, 2026-09-15), `CALC.ELF` nie kompiluje się bez tego w co najmniej jednym środowisku (używa `bool`/`true`/`false`). Jednolinijkowy, bezpieczny fix.
- [ ] Naprawić link `HELLO.ELF` pod nowszym binutils — `ld -Ttext 0x400000 ...` (patrz `scripts/build.sh`) kończy się `section .note.gnu.property LMA [...] overlaps section .text LMA [...]` w co najmniej jednym środowisku (zgłoszone dwukrotnie, na lokalnej maszynie Kocurowy96 linkuje się OK, więc to kwestia wersji binutils, nie logiki). Do wypróbowania: `-Wl,--no-rosegment` przy linkowaniu, albo `objcopy --remove-section=.note.gnu.property` po kompilacji, albo podniesienie adresu `-Ttext` dla HELLO (dziś jedyna apka na `0x400000`, najniższym z całej serii `0x400000`-`0xC00000` — inne nie mają problemu). Zweryfikować że nadal działa też lokalnie po zmianie.
- [ ] Ścieżka do binarki Limine w `scripts/make_disk.sh`/`scripts/run.sh` zakłada `limine_dir/limine`, ale świeże `configure && make` (przynajmniej w środowisku agenta, Limine 12.5.2) produkuje `limine_dir/bin/limine` — zgłoszone 2026-09-15. Dodać fallback (sprawdzić które z dwóch istnieje) zamiast twardej ścieżki, żeby działało w obu układach.
